...
export const AttributesForm = () => {
    ...
    const{selectedAttribute,closeForm,updateAttribute,createAttribute,attributevalues,addAttributevalue,deleteAttribute,attributes}=AttributeStore;
...
*/
    ...
    const handleinputchange =(event)=>{
        ...
        setAttribute({...attribute,[event.currentTarget.name]:event.currentTarget.value});
    };
    ...
    const validationSchema=yup.object({
        name:yup.string().matches(/^[a-zA-Z0-9]{3,}$/,'emri duhet te ket mbi 3 shkronja').required("Ju lutem shenoni emrin "),
    ...
    return (
     ...
     <Formik validationSchema={validationSchema}
            enableReinitialize 
            initialValues={attribute} onSubmit={values => handleFormsubmit(values)}>
